...
class DownBLock(nn.Module):
    def __init__(self, in_features, out_features, index, reduce_size: bool):
        ...
        conv_block = [
            nn.Conv3d(in_features, out_features, kernel_size=3),  # Same padding
            nn.InstanceNorm3d(out_features),
            nn.ReflectionPad3d(1),
            nn.PReLU(out_features),
            nn.Conv3d(out_features, out_features, kernel_size=3),  # Same padding
            nn.InstanceNorm3d(out_features),
            nn.ReflectionPad3d(1),
            nn.PReLU(out_features),
        ]
        if reduce_size:
            self.pool = nn.Conv3d(out_features, out_features, kernel_size=2, stride=2)
        else:
...
class BridgeBlock(nn.Module):
    def __init__(self, in_features):
        ...
        conv_block = [
            nn.Conv3d(in_features, in_features * 2, kernel_size=3),
            nn.InstanceNorm3d(in_features * 2),
            nn.ReflectionPad3d(1),
            nn.Dropout(),
            nn.ReLU(inplace=True),
            nn.Conv3d(in_features * 2, in_features, kernel_size=3),
...
class UpBLock(nn.Module):
    def __init__(self, in_features, out_features, index, increase_size=True, drop_out=0.5):
        ...
        conv_block = [
            nn.Conv3d(2 * in_features, in_features, kernel_size=3),
            nn.InstanceNorm3d(in_features),
            nn.ReflectionPad3d(1),
...
class UNet(nn.Module):
        ...
        for i in range(self.num_downs):
            ...
            ups.append(
                UpBLock(
                    ...
                    j,
                    j < reduce_till,
                    drop_out=net_G_drop_out if i != self.num_downs - 1 else 0,
                )
            )
            in_features = out_features
            out_features //= 2

        self.lastLayer = nn.Sequential(nn.Conv3d(in_features, output_nc, 1, padding=0), nn.Tanh())
    ...
    def forward(self, x, return_intermediate=False, layers=[]):
        ...
        tmp = x
        for i in range(self.num_downs):
            print(tmp.shape) if self.first else None
