...
class Generator(nn.Module):
    def __init__(self, input_nc, output_nc, net_G_depth=9, net_G_downsampling=2, net_G_channel=64, net_G_drop_out=0.5, **args):
        ...
        model = [
            ...
            nn.Conv3d(input_nc, net_G_channel, 7),
            nn.InstanceNorm3d(net_G_channel),
            nn.ReLU(inplace=True),
        ]
        ...
        for _ in range(net_G_downsampling):
            model += [
                nn.ConvTranspose3d(in_features, out_features, 3, stride=2, padding=1, output_padding=1),
                nn.InstanceNorm3d(out_features),
                nn.ReLU(inplace=True),
            ]
            in_features = out_features
        ...
        # Output layer
        model += [nn.ReflectionPad3d(3), nn.Conv3d(in_features, output_nc, 7), nn.Tanh()]
        # model += [nn.Conv3d(in_features, output_nc, 1), nn.Tanh()]
